...
public class ArrayMultidimensional {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < M.length; i++) { //navegando na matriz
            ...
            for (int j = 0; j < M.length; j++) { //navegando na linha (array unidimensional)
                ...
                if (M[i][j] < menor) {
                    menor = M[i][j]; //valor da variável menor passa a ser o valor aleatório
                    linha = i; // que está na linha representada pela variável i
                    linha = j; //que está na coluna representada pela variável
                }
